...
export const getListaValoresReprogramados = async (dreUuid, status_padrao) => {
    return (
        await api.get(`api/valores-reprogramados/?dre_uuid=${dreUuid}&status=${status_padrao}`, authHeader)
    ).data
}

export const filtrosListaValoresReprogramados = async(dreUuid, search=null, tipo_unidade=null, status_valores=null) => {
    const url = `api/valores-reprogramados/?dre_uuid=${dreUuid}&search=${search}&tipo_unidade=${tipo_unidade}&status=${status_valores}`

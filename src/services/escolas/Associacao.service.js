...
export const editarMembroAssociacao = async (payload, uuid) => {
  return await api.put(
    `/api/membros-associacao/${uuid}/?associacao_uuid=${localStorage.getItem(
      ASSOCIACAO_UUID
    )}`,
    payload,
    authHeader()
...
export const patchStatusPresidenteAssociacao = async (
  associacao_uuid,
  payload
) => {
  return (
...
export const reenviarSolicitacaoEncerramentoConta = async (
  payload,
  idSolicitacao
) => {
...
export const exportarDadosAssociacaoPdf = async () => {
  return api
    .get(
      `/api/associacoes/${localStorage.getItem(ASSOCIACAO_UUID)}/exportar-pdf`,
      {
        responseType: "blob",
        timeout: 30000,
        ...authHeader(),

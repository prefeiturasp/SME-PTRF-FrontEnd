...
export const Lista = () => {
  ...
  const { setShowModalForm, stateFormModal, setStateFormModal, setBloquearBtnSalvarForm, forceLoading } =
    useContext(MandatosContext);
  ...
  return (
    <>
      {results && results.length > 0 ? (
        <div className="p-2">
          <DataTable value={results} className="tabela-lista-usuarios">
            <Column field="referencia_mandato" header="Referencia Mandato" />
            <Column field="data_inicial" header="Data inicial" body={dataTemplate} />
            <Column field="data_final" header="Data Final" body={dataTemplate} />
            <Column field="acao" header="Ação" body={acoesTemplate} style={{ width: "10%", textAlign: "center" }} />
          </DataTable>
        </div>
      ) : (
        <div className="p-2">

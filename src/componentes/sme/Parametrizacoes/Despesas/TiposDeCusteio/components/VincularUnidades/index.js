...
export const VincularUnidades = ({ UUID, podeEditar }) => {
  ...
  const { mutationVincularUnidadeEmLote } = useVincularUnidade();
  ...
  const handleVincularUnidades = async (rowData) => {
    try {
      ...
      mutationVincularUnidadeEmLote.mutate({ uuid: UUID, unidadeUUID: uuids });
      setSelectedUnidades([]);
    } catch (error) {
      console.error(error);
    }
  ...
  const handleConfirmarVincularUnidade = (rowData) => {
    ModalConfirm({
      ...
      dataQa: "modal-confirmar-vincular-unidade-ao-tipo-de-despesa-custeio",

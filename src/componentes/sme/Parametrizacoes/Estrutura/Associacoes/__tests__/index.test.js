...
describe("Teste handleSubmitModalForm", () => {
  ...
  it("teste criação sucesso", async () => {
    getParametrizacoesAssociacoes
      .mockResolvedValueOnce(mockListaAssociacoes)
      .mockResolvedValueOnce(mockListaAssociacoes);
    getUnidadePeloCodigoEol.mockReturnValue({
      tipo_unidade: "CEI",
      nome: "Unidade 007",
    });
    ...
    const input_data_encerramento = screen.getByLabelText(
    ...
    fireEvent.change(input_periodo_inicial, {
      target: { value: "1e8c492b-2edb-4acd-a808-71bdf0d805d5" },
    });
...
describe("Teste handle functions", () => {
  ...
  it("test onPageChange", async () => {
    getParametrizacoesAssociacoes
      .mockResolvedValueOnce(mockListaAssociacoes)
      .mockResolvedValueOnce(mockListaAssociacoesPaginated);
    visoesService.getPermissoes.mockResolvedValue(true);
    render(
      <MemoryRouter initialEntries={["/parametro-associacoes"]}>
        <Routes>
    ...
    await waitFor(() => {
      expect(
        screen.queryByText(
          "ASSOCIACAO DE PAIS E MESTRES DO CEI DR ANTONIO JOAO SILVA"
        )
      ).toBeInTheDocument();
      expect(
        screen.queryByText("ASSOCIACAO DE PAIS E MESTRES DO CECI CEI JARAGUA")
  ...
  it("test envia filtros", async () => {
    ...
    getParametrizacoesAssociacoes
      .mockResolvedValueOnce(mockListaAssociacoes)
      .mockResolvedValueOnce(mockListaAssociacoesFiltro);
    visoesService.getPermissoes.mockResolvedValue(true);
    render(
      <MemoryRouter initialEntries={["/parametro-associacoes"]}>
        <Routes>
    ...
    await waitFor(() => {
      ...
      fireEvent.change(input, {
        target: {
          ...
          value: "ASSOCIACAO DE PAIS E MESTRES DO CECI CEI JARAGUA",
        },
      });
      expect(input.value).toEqual(
        "ASSOCIACAO DE PAIS E MESTRES DO CECI CEI JARAGUA"
      );
    });

...
export const ContasDasAssociacoes = () => {
  ...
  const [todasAsAssociacoesAutoComplete, setTodasAsAssociacoesAutoComplete] =
    useState([]);
  ...
  const carregaTodasAsContas = useCallback(
    async (
      page = 1,
      filtrar_associacao = "",
      filtrar_por_tipo_conta = "",
      ...
      setLoading(true);
      let todas_contas = await getContasAssociacoesFiltros(
        page,
        filtrar_associacao,
        filtrar_por_tipo_conta,
        filtrar_por_status
  ...
  const totalDeContas = useMemo(
    () => todasAsContas?.count || 0,
    [todasAsContas]
  );
  ...
  const handleSubmitFiltros = async () => {
    ...
    let contas_filtradas = await getContasAssociacoesFiltros(
      1,
  ...
  const handleSubmitModalForm = async (stateFormModal) => {
    ...
    if (stateFormModal.operacao === "create") {
      try {
        ...
        if (e.response.data && e.response.data.non_field_errors) {
          ...
          setErroExclusaoNaoPermitida(
            "Erro ao criar conta de associacao. Tente novamente."
          );
          setShowModalInfoUpdateNaoPermitido(true);
        }
      }
      ...
      try {
        ...
        setShowModalForm(false);
        toastCustom.ToastCustomSuccess(
          "Edição da ação da associação realizada com sucesso.",
          `A ação da associação foi editada no sistema com sucesso.`
        );
        await carregaTodasAsContas();
      } catch (e) {
        if (e.response.data && e.response.data.non_field_errors) {
          ...
          setErroExclusaoNaoPermitida(
            "Erro ao atualizar conta de associacao. Tente novamente."
          );
          setShowModalInfoUpdateNaoPermitido(true);
        }
      }
  ...
  const onDeleteAcaoTrue = async () => {
    try {
      ...
      setShowModalForm(false);
      toastCustom.ToastCustomSuccess(
        "Remoção da ação da associação efetuada com sucesso.",
        `A ação da associação foi removida do sistema com sucesso.`
      );
      await carregaTodasAsContas();
    } catch (e) {
      if (e.response.data && e.response.data.mensagem) {
        ...
        setMensagemModalInfoNaoPodeExcluir(
          "Houve um problema ao realizar esta operação, tente novamente."
        );
        setShowModalInfoNaoPodeExcluir(true);
        setShowModalDeleteAcao(false);
      }
  ...
  return (
    <PaginasContainer>
      ...
      <div className="page-content-inner">
        <MenuInterno caminhos_menu_interno={UrlsMenuInterno} />
        {loading ? (
          <div className="mt-5">
            <Loading
              corGrafico="black"
          ...
          <>
            ...
            {(todasAsContas?.results || []).length ? (
              <>
                <p>
                  Exibindo <span className="total-acoes">{totalDeContas}</span>{" "}
                  contas de associações
                </p>
                <Tabela
        ...
        <section>
          <ModalConfirmarExclusao
            ...
            bodyText={
              <p>Tem certeza que deseja excluir esta conta de associação?</p>
            }
          />
        </section>
      </div>

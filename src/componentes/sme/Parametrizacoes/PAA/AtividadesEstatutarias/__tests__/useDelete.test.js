...
jest.mock("../../../../../../services/sme/Parametrizacoes.service", () => ({
    deleteAtividadesEstatutarias: jest.fn(),
}));
...
describe("useDelete", () => {
    ...
    const wrapper = ({ children }) => (
        <QueryClientProvider client={queryClient}>
            <AtividadesEstatutariasContext.Provider value={{ setShowModalForm, setBloquearBtnSalvarForm }}>
                {children}
    ...
    it("deve excluir um atividade estatutária com sucesso", async () => {
        deleteAtividadesEstatutarias.mockResolvedValueOnce({});
        ...
        expect(deleteAtividadesEstatutarias).toHaveBeenCalledWith("uuid-fake");
        expect(setShowModalForm).toHaveBeenCalledWith(false);
        expect(toastCustom.ToastCustomSuccess).toHaveBeenCalledWith(
            "Sucesso!",
            "Atividade Estatutária excluída com sucesso."
        );
    });

    it("deve exibir mensagem de erro quando a API retorna uma mensagem de erro", async () => {
        deleteAtividadesEstatutarias.mockRejectedValueOnce({
    ...
    it("deve exibir mensagem de erro quando a API retorna um non_fields de erro", async () => {
        ...
        expect(toastCustom.ToastCustomError).toHaveBeenCalledWith(
            "Erro!",
            "Não foi possível excluir a atividade estatutária.");
    });

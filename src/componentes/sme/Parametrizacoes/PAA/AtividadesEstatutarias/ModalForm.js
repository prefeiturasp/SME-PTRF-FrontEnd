...
export const ModalForm = ({
  ...
  const statusOptions = (atividadesEstatutariasTabelas?.status || []).map(
    (item) => ({
      value: item.key,
      label: item.value,
    }),
  ...
  const tiposOptions = (atividadesEstatutariasTabelas?.tipo || []).map(
    (item) => ({
      value: item.key,
      label: item.value,
    }),
  ...
  const anoOptions = (atividadesEstatutariasTabelas?.ano || []).map((item) => ({
    value: item.key,
    label: item.value,
  }));
              ...
              <Row gutter={16}>
                ...
                <Col span={12}>
                  <div className="form-group">
                    ...
                    />
                    {props.touched.mes && props.errors.mes && (
                      <span className="span_erro text-danger mt-1">
                        {" "}
                        {props.errors.mes}{" "}
                      </span>
                ...
                <Col span={12}>
                  <div className="form-group">
                    ...
                    <Select
                      ...
                      style={{ width: "100%" }}
                      options={anoOptions}
                      onChange={(e) => setFieldValue("ano", e)}
                      disabled={
                        !TEM_PERMISSAO_EDICAO_PAINEL_PARAMETRIZACOES ||
                        mesOptions.length === 0
                      }

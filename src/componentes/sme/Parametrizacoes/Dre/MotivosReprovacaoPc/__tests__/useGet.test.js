...
jest.mock("../../../../../../services/sme/Parametrizacoes.service", () => ({
    getMotivosReprovacaoPc: jest.fn(),
}));

describe("useGetMotivosReprovacaoPc", () => {
    let queryClient;
    ...
    const renderCustomHook = (filterValue) => {
        return renderHook(() => useGetMotivosReprovacaoPc(), {
                wrapper: ({ children }) => (
                    <QueryClientProvider client={queryClient}>
                        <MotivosReprovacaoPcContext.Provider value={{ filter: { motivo: filterValue } }}>
                            {children}
    ...
    test("deve retornar dados corretamente quando a API retorna sucesso", async () => {
        const mockData = [{ id: 1, motivo: "Motivo 1", uuid: "123", recurso_uuid: "r1" }];
        getMotivosReprovacaoPc.mockResolvedValue({results: mockData, count: 1});
    ...
    test("deve lidar com erro da API corretamente", async () => {
        getMotivosReprovacaoPc.mockRejectedValue(new Error("Erro na API"));

...
export const Filtros = () => {
    const { filter, setFilter, tabelas } = useContext(AcertosLancamentosContext);
    const { Option } = Select;
    ...
    const [filtroTemporario, setFiltroTemporario] = useState({
        ...
        filtrar_por_categoria: filter.filtrar_por_categoria?.length > 0 ? filter.filtrar_por_categoria : [""],
        filtrar_por_ativo: filter.filtrar_por_ativo,
    });

    // Sincroniza o filtro temporário quando o filtro do contexto muda
    useEffect(() => {
        setFiltroTemporario({
            filtrar_por_nome: filter.filtrar_por_nome,
            filtrar_por_categoria: filter.filtrar_por_categoria?.length > 0 ? filter.filtrar_por_categoria : [""],

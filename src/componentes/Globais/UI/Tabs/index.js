...
const Tabs = ({ tabs, initialActiveTab, onTabClick, identifier='nav' }) => {
    ...
    return (
        <nav>
            <div className="nav nav-tabs mb-3 menu-interno-dre-prestacao-de-contas" role="tablist">
                {tabs.map((tab, index) => (
                    <Fragment key={index}>
                        <a
                            onClick={() => handleTabClick(tab.uuid)}
                            className={`nav-link btn-escolhe-acao ${activeTab === tab.uuid ? "btn-escolhe-acao-active" : ""}`}

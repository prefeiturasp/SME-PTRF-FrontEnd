...
import LogoSP from "../../../assets/img/logos/logo_PrefSP_sem fundo_horizontal_branco_monocromatico.png";
...
export const SidebarLeft = () => {
  ...
  const getPathname = () => {
    ...
    return getPathnameExcecoes(array_pathname[1])
      ? getPathnameExcecoes(array_pathname[1])
      : array_pathname[1];
  };
    ...
    <>
      ...
      >
        ...
        <SideNav.Nav defaultSelected={getPathname()}>
          {urls &&
            urls.lista_de_urls.length > 0 &&
            urls.lista_de_urls.map((url, index) => {
              let featureFlag = true;
              if (url.featureFlag) {
                featureFlag = visoesService.featureFlagAtiva(url.featureFlag);
              }

              return featureFlag && validaPermissao(url) ? (
                <NavItem
                  key={index}
                  navitemClassName={`d-flex align-items-end ${
                    url.subItens && url.subItens.length > 0 ? "sub-menu" : ""
                  }`}
                  data-tip={url.label}
                  data-for={url.dataFor}
                  eventKey={url.url}
                >
                  <NavIcon>
                  ...
                  <ReactTooltip
                    disable={sidebarStatus.sideBarStatus}
                    id={url.dataFor}
                  >
                    {}
                  </ReactTooltip>
                  {url.subItens &&
                    url.subItens.length > 0 &&
                    url.subItens.map((subItem, index) =>

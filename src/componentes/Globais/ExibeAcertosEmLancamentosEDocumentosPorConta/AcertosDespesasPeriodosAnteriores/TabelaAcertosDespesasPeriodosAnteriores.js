...
export const TabelaAcertosDespesasPeriodosAnteriores = ({
  ...
  const [
    showModalJustificarNaoRealizacao,
    setShowModalJustificarNaoRealizacao,
  ] = useState(false);
  const [showModalJustificadaApagada, setShowModalJustificadaApagada] =
    useState(false);
  const [textoConfirmadoJustificado, setTextoConfirmadoJustificado] =
    useState("");

  const [tipoAcao, setTipoAcao] = useState("");

  let dados_analise_dre_usuario_logado =
    meapcservice.getAnaliseDreUsuarioLogado();

  // Paginação
  const [primeiroRegistroASerExibido, setPrimeiroRegistroASerExibido] =
    useState(
      dados_analise_dre_usuario_logado
        .conferencia_de_despesas_periodos_anteriores.paginacao_atual
        ? dados_analise_dre_usuario_logado
            .conferencia_de_despesas_periodos_anteriores.paginacao_atual
        : 0
    );
  ...
  const tagJustificativa = (rowData) => {
    ...
    if (statusId && statusId !== "PENDENTE") {
      let nomeStatus = opcoesJustificativa.status_realizacao.find(
        (justificativa) => justificativa.id === statusId
      );
        ...
        >
          <div className="row">
            <div className="col-5">
              ...
              / {totalDeAcertosDosLancamentos} totais
            </div>
  ...
  const modalBodyHTML = () => {
    return (
      <div className="modal-body">
        <form>
          <label htmlFor="justifique-textarea">
            Justifique abaixo a não realização do acerto no lançamento.
          </label>
          <textarea
  ...
  const salvaEstadoPaginacaoLancamentosLocalStorage = (event) => {
    dados_analise_dre_usuario_logado.conferencia_de_despesas_periodos_anteriores.paginacao_atual =
      event.rows * event.page;
    dados_analise_dre_usuario_logado.conferencia_de_despesas_periodos_anteriores.expanded =
      expandedRowsLancamentos;
    meapcservice.setAnaliseDrePorUsuario(
  ...
  return (
    <>
      ...
      {lancamentosAjustes && lancamentosAjustes.length > 0 ? (
        ...
        >
          <Column header="Ver Acertos" expander style={{ width: "6%" }} />
          <Column
            field="data"
            header="Data"
            body={dataTemplate}
          ...
          />
          {visoesService.getItemUsuarioLogado("visao_selecionada.nome") ===
            "UE" &&
          visoesService.getPermissoes(["change_analise_dre"]) &&
        ...
        <p className="text-center fonte-18 mt-4">
          <strong>
            Não foram solicitados acertos nas despesas de períodos anteriores
            nessa análise da PC.
          </strong>
        </p>
      )}
      ...
      <section>
        <ModalJustificarNaoRealizacao
          ...
          primeiroBotaoOnClick={() =>
            setShowModalJustificarNaoRealizacao(false)
          }
      ...
      <section>
        <ModalJustificadaApagada
          ...
          texto={
            "Atenção. Essa ação irá apagar as justificativas digitadas. Confirma ação?"
          }
          primeiroBotaoTexto="Confirmar"
          primeiroBotaoCss="success"
          primeiroBotaoOnclick={() =>
            tipoAcao === "limpar_status"
              ? limparStatus()
              : marcarComoRealizado()
          }

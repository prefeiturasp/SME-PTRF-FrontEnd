...
const AcertosDespesasPeriodosAnteriores = ({
    ...
    useEffect(() => {
        ...
        let expanded_uuids = dados_analise_dre_usuario_logado.conferencia_de_despesas_periodos_anteriores.expanded
    ...
    const carregaDadosDasContasDaAssociacao = useCallback(async () => {
        ...
        if (prestacaoDeContas && prestacaoDeContas.associacao && prestacaoDeContas.associacao.uuid && analiseAtualUuid) {
            let contas = await getContasDaAssociacaoComAcertosEmDespesasPeriodosAnteriores(prestacaoDeContas.associacao.uuid, analiseAtualUuid);
            setContasAssociacao(contas);

            // Para Manter o Estado
            if (contas.length === 1){
                let dados_analise_dre_usuario_logado = meapcservice.getAnaliseDreUsuarioLogado()
                dados_analise_dre_usuario_logado.conferencia_de_despesas_periodos_anteriores.conta_uuid = contas[0].uuid
    ...
    const carregaAcertosLancamentos = useCallback(async (conta_uuid, filtrar_por_lancamento = null, filtrar_por_tipo_de_ajuste = null) => {
        ...
        let lancamentos_ajustes = await getDespesasPeriodosAnterioresAjustes(analiseAtualUuid, conta_uuid, filtrar_por_lancamento, filtrar_por_tipo_de_ajuste)
    ...
    useEffect(() => {
        if (contasAssociacao && contasAssociacao.length > 0) {
            ...
            if(dados_analise_dre_usuario_logado && dados_analise_dre_usuario_logado.conferencia_de_despesas_periodos_anteriores && dados_analise_dre_usuario_logado.conferencia_de_despesas_periodos_anteriores.conta_uuid){
                carregaAcertosLancamentos(dados_analise_dre_usuario_logado.conferencia_de_despesas_periodos_anteriores.conta_uuid)
                setContaSelecionada(dados_analise_dre_usuario_logado.conferencia_de_despesas_periodos_anteriores.conta_uuid)

                // necessário para selecionar a aba de conta corretamente
                let index_encontrado = null;
                for(let i=0; i<=contasAssociacao.length-1; i++){
                    if(contasAssociacao[i].uuid === dados_analise_dre_usuario_logado.conferencia_de_despesas_periodos_anteriores.conta_uuid){
                        index_encontrado = i;
                        break;
                    }
                }
    ...
    const toggleBtnEscolheConta = (id) => {
        ...
        let dados_analise_dre_usuario_logado = meapcservice.getAnaliseDreUsuarioLogado();
        dados_analise_dre_usuario_logado.conferencia_de_despesas_periodos_anteriores.paginacao_atual = 0
        dados_analise_dre_usuario_logado.conferencia_de_despesas_periodos_anteriores.expanded = []
        meapcservice.setAnaliseDrePorUsuario(visoesService.getUsuarioLogin(), dados_analise_dre_usuario_logado)
    ...
    const salvaObjetoAnaliseDrePorUsuarioLocalStorage = (conta_uuid_lancamentos) =>{
        ...
        objetoAnaliseDrePorUsuario.conferencia_de_despesas_periodos_anteriores.conta_uuid = conta_uuid_lancamentos
...
export default memo(AcertosDespesasPeriodosAnteriores)
...
const BotaoAcertosDocumentosInclusaoCredito = ({
  analise_documento,
  prestacaoDeContasUuid,
  prestacaoDeContas,
  analisePermiteEdicao,
  uuid_acerto_documento,
  acerto,
}) => {
  ...
  return (
    <>
      {acerto && !acerto.receita_incluida && TEMPERMISSAO ? (
        ...
        >
          <>
            <FontAwesomeIcon style={{ fontSize: "16px" }} icon={faCheckCircle} />
            <strong>
              {" "}
              Crédito incluído. <span className="clique-aqui-atualizada">Clique aqui para editar</span>
            </strong>
          </>
        </LinkCustom>
      ) : (
        acerto &&
        acerto.receita_incluida &&
        !TEMPERMISSAO && (
          <LinkCustom
            url={URL}
            analise_documento={analise_documento}
          ...
          >
            <>
              ...
              <strong>
                {" "}
                Crédito incluído. <span className="clique-aqui-atualizada">Clique aqui para ver</span>
              </strong>
            </>
          </LinkCustom>
        )

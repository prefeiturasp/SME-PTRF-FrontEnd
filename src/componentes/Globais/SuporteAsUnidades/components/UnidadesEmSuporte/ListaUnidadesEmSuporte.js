...
export const UnidadesEmSuporte = () => {
    ...
    const {mutationRemoverSuporte, mutationRemoverSuporteEmLote} = useRemoverSuporte()
    ...
    function handleRemoverSuporte(rowData) {
        mutationRemoverSuporte.mutate({usuario: getUsuarioLogado().login, unidade_uuid: rowData.uuid});
    };

    const handleEncerrarSuporteEmLote = async () => {
        try {
            const uuids = selectedUnidades.map((item) => item.uuid);
            await mutationRemoverSuporteEmLote.mutateAsync({usuario: getUsuarioLogado().login,  unidade_uuids: uuids});
            setSelectedUnidades([]);
          } catch (error) {
            console.error(error)
          }
    ...
    const handleConfirmarRemoverSuporteEmLote = (rowData) => {
        ModalConfirm({
            dispatch,
            title: 'Confirmação de encerramento de suporte',
            message: '<p>Deseja encerrar o suporte a essas unidades?</p> <p> Ao confirmar, você não visualizará mais essas unidades como suporte.</p>',
            cancelText: 'Não',
            confirmText: 'Sim',
            confirmButtonClass: "btn-danger",
                        ...
                        <div className="col-7">
                            <div className="row">
                                <div className="col-12">
                                    <a className="float-right" style={{textDecoration:"underline", cursor:"pointer"}} onClick={() => setSelectedUnidades([])}>
                                        <FontAwesomeIcon
                                    ...
                                    <a className="float-right" style={{textDecoration:"underline", cursor:"pointer"}} onClick={() => handleConfirmarRemoverSuporteEmLote()}>
                                        <FontAwesomeIcon
                                            style={{color: "white", fontSize: '15px', marginRight: "2px"}}
                                            icon={faTrash}
                                        />

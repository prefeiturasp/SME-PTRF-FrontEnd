...
export const InfosContas = ({ dadosDaAssociacao }) => {
  ...
  const confirmarEncerramentoConta = async () => {
    if (
      dataModalConfirmarEncerramentoConta.conta &&
      dataModalConfirmarEncerramentoConta.conta.uuid &&
      dataModalConfirmarEncerramentoConta.conta.solicitacao_encerramento.uuid
    ) {
  ...
  const handleRejeitarEncerramentoConta = (motivosRejeicao = [], outrosMotivosRejeicao = "") => {
    ...
    if (motivosRejeicao && motivosRejeicao.length > 0) {
      motivosSelecionados = motivosRejeicao.filter((obj) => obj.selected).map((obj) => obj.uuid);
    }
  ...
  const rejeitarEncerramentoConta = async (payloadMotivos) => {
    if (
      dataModalRejeitarEncerramentoConta.conta &&
      dataModalRejeitarEncerramentoConta.conta.uuid &&
      dataModalRejeitarEncerramentoConta.conta.solicitacao_encerramento.uuid
    ) {
        ...
        <>
          ...
          <div className="row">
            {contasDaAssociacao && contasDaAssociacao.length > 0 ? (
              ...
              <MsgImgCentralizada texto={mensagemQuandoNaoExistemContasParaApresentar(contasEncerradas)} img={Img404} />
            )}
          </div>

          {contasEncerradas && contasEncerradas.length > 0 && (
            <section className="mt-5">
              <TabelaContasEncerradas contas={contasEncerradas} rowsPerPage={10} />
            </section>
          )}

...
export const RegularidadeAssociacoes = () => {
  ...
  const unidadeEscolarTemplate = (rowData) => {
    return (
      <div>
        {rowData["associacao"]["unidade"]["nome_com_tipo"] ? (
          <strong>{rowData["associacao"]["unidade"]["nome_com_tipo"]}</strong>
        ) : (
          ""
        )}
      </div>
    ...
    return (
      <>
        <button
          ...
          disabled={visoesService.getPermissoes(["access_regularidade_dre"]) ? false : true}
        >
  ...
  const handleSubmitFiltrosAssociacao = async (event) => {
    ...
    let resultado_filtros = await filtrosRegularidadeAssociacoes(
      stateFiltros.unidade_escolar_ou_associacao,
      stateFiltros.regularidade,
      stateFiltros.tipo_de_unidade,
      stateFiltros.ano,
    );
  ...
  return (
    <>
      ...
      {loading ? (
        ...
        <MsgImgCentralizada texto="Não encontramos resultados, verifique os filtros e tente novamente." img={Img404} />
      ) : (
        <MsgImgLadoDireito texto="Não encontramos nenhuma Associação com este perfil, tente novamente" img={Img404} />
      )}
    </>
  );
};

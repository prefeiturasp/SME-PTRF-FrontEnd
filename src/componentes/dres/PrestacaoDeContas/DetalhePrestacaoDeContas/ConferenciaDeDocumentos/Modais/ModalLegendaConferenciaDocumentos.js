...
export const ModalLegendaConferenciaDocumentos = (propriedades) => {
    ...
    const handleTagConferencia = useCallback(async () => {
        setLoading(true)
        try {
            const response = await getTagsConferenciaDocumento()
            setListaTagsConferencia(response)
        } catch (e) {
            console.error('Erro ao carregar tags de conferência de documentos', e)
        }
        setLoading(false)

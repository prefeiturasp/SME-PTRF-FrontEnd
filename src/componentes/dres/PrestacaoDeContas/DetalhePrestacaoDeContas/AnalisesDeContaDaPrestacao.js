...
export const AnalisesDeContaDaPrestacao = ({
  infoAta,
  ...
  const tooltip_1 =
    "A diferença entre o saldo bancário e o saldo reprogramado <br/> pode ocorrer em virtude de cheques não compensados, <br/> despesas/créditos não lançados ou lançados com erro ou <br/> estornos ainda não realizados.";

  const [
    showModalVisualizarArquivoDeRerefencia,
    setShowModalVisualizarArquivoDeRerefencia,
  ] = useState(false);
  ...
  const handleClickDownloadArquivoDeReferencia = useCallback(
    async (rowData) => {
      let nome_do_arquivo = `${rowData.nome}`;
      try {
        await getDownloadArquivoDeReferencia(
          nome_do_arquivo,
          rowData.uuid,
          rowData.tipo
        );
  ...
  const handleClickVisualizarArquivoDeReferencia = useCallback(
    (extrato_bancario) => {
  ...
  const informacoes_conciliacao_ue = () => {
    ...
    if (infoAta && infoAta.conta_associacao) {
      ...
      info = prestacaoDeContas.informacoes_conciliacao_ue.find(
        (element) => element.conta_uuid === conta_associacao
      );

      let arquivos_referencia = prestacaoDeContas.arquivos_referencia.filter(
        (element) => element.conta_uuid === infoAta.conta_associacao.uuid
      );
      extrato_bancario = arquivos_referencia.filter(
        (element) => element.tipo === "EB"
      );
    }
    ...
    if (info) {
      ...
      dados.data_extrato = info.data_extrato
        ? moment(info.data_extrato).format("DD/MM/YYYY")
        : "-";
      dados.extrato_bancario =
        extrato_bancario.length > 0 ? extrato_bancario[0] : null;
    }
  ...
  const calculaDiferencaUe = () => {
    if (informacoes_conciliacao_ue().saldo_extrato !== "-") {
      let diferenca =
        infoAta.totais.saldo_atual_total -
        trataNumericos(informacoes_conciliacao_ue().saldo_extrato);
  ...
  const calculaDiferencaDre = () => {
    if (index > -1) {
      if (
        trataNumericos(analisesDeContaDaPrestacao[index].saldo_extrato) ||
        trataNumericos(analisesDeContaDaPrestacao[index].saldo_extrato) === 0
      ) {
        let diferenca =
          infoAta.totais.saldo_atual_total -
          trataNumericos(analisesDeContaDaPrestacao[index].saldo_extrato);
  ...
  const permiteSalvar = () => {
    ...
    if (index > -1) {
      if (
        ...
        !analisesDeContaDaPrestacao[index]
      ...
      if (
        formErrosAjusteSaldo &&
        formErrosAjusteSaldo[index] &&
        formErrosAjusteSaldo[index].data
      ) {
        permite = false;
      }
    } else {
            ...
            <div className="col-12">
              <div className="row container-titulo-extrato-bancario ml-0 mr-0">
                <div className="col-12 align-self-center">
                  <span className="titulo-extrato-bancario">
                    Extrato Bancário da Unidade
                  </span>
                </div>
              </div>
              ...
              <div className="row container-extrato-bancario ml-0 mr-0">
                ...
                <div className="col-4 mt-3">
                  ...
                  <p className="dados-extrato-bancario mt-3">
                    {informacoes_conciliacao_ue().saldo_extrato !== "-"
                ...
                <div className="col-5 mt-3">
                  ...
                  <p
                    className={`dados-extrato-bancario mt-3 ${
                      temDiferencaUe() ? "tem-diferenca" : ""
                    }`}
                  >
                    {!temDiferencaUe() && (
                      <FontAwesomeIcon
                ...
                <div className="row container-extrato-bancario ml-0 mr-0">
                  <div className="col-12 mt-3">
                    <div className="d-flex align-items-center justify-content-start pb-3">
                      <span className="pr-2">
                        {informacoes_conciliacao_ue().extrato_bancario.nome}
                      </span>
                      <button
                        onClick={() =>
                          handleClickVisualizarArquivoDeReferencia(
                            informacoes_conciliacao_ue().extrato_bancario
                          )
                        }
                      ...
                      <button
                        onClick={() =>
                          handleClickDownloadArquivoDeReferencia(
                            informacoes_conciliacao_ue().extrato_bancario
                          )
                        }
                        className="btn-editar-membro"
              ...
              {(adicaoAjusteSaldo ||
                (index > -1 &&
                  analisesDeContaDaPrestacao[index] &&
                  analisesDeContaDaPrestacao[index].uuid)) && (
                <div id="correcao_extrato_bancario">
                  ...
                  <div className="row container-extrato-bancario ml-0 mr-0">
                    ...
                    <div className="col-4 mt-2 mb-4">
                      <label htmlFor="saldo_extrato">
                        <strong>Saldo corrigido</strong>
                      </label>
                      <CurrencyInput
                        disabled={!editavel || !adicaoAjusteSaldo}
                        allowNegative={true}
                      ...
                      />
                      {formErrosAjusteSaldo[index] &&
                        formErrosAjusteSaldo[index].saldo && (
                          <span className="span_erro text-warning mt-1">
                            {" "}
                            {formErrosAjusteSaldo[index].saldo}
                          </span>
                        )}
                    ...
                    <div className="col-5 mt-2 mb-4">
                      <span className="">
                        <strong>
                          Diferença em relação à prestação de contas
                        </strong>
                      </span>
                      ...
                      <div className="d-flex bd-highlight dados-extrato-bancario-correcao">
                        <div className="flex-grow-1 bd-highlight mt-3">
                          <span
                            className={` ${
                              temDiferencaAjusteDre() ? "tem-diferenca" : ""
                            }`}
                          >
                            {!temDiferencaAjusteDre() &&
                              calculaDiferencaDre() !== "-" && (
                  ...
                  <div className="row container-extrato-bancario ml-0 mr-0">
                    <div className="col-12">
                      <Form.Check
                        ...
                        label={
                          "Solicitar o envio do comprovante de saldo da conta"
                        }
                        key={index}
                        onChange={(e) => {
                          handleChangeAnalisesDeContaDaPrestacao(
                    ...
                    <div className="d-flex bd-highlight">
                      ...
                      {index > -1 && editavel && (
                        <div className="bd-highlight">
                          <button
                            type="button"
                            className="btn btn btn-danger mt-3 mb-3 mr-2"
                            onClick={() => {
                              onClickDeletarAcertoSaldo();
                            }}
                          >

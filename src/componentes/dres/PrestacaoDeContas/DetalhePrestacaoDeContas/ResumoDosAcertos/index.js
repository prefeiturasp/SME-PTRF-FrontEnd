...
export const ResumoDosAcertos = () => {
    ...
    const [totalDespesasPeriodosAnterioresAjustes, setTotalDespesasPeriodosAnterioresAjustes] = useState(undefined)    
    ...
    const [abaAtivaResumoDosAcertos, setAbaAtivaResumoDosAcertos] = useState('')
    const [abaSelecionadaManualmente, setAbaSelecionadaManualmente] = useState(false)
    ...
    const getAnalisePrestacao = useCallback(()=>{
        if (prestacaoDeContas) {
            let arrayAnalises = [];
            if (prestacaoDeContas?.analises_de_conta_da_prestacao && prestacaoDeContas.analises_de_conta_da_prestacao.length > 0){
                prestacaoDeContas.analises_de_conta_da_prestacao.map((conta)=>{
                        arrayAnalises.push({
                            ...
                            saldo_extrato: conta.saldo_extrato === null ? null : valorTemplate(conta.saldo_extrato),
                        })
                    });
    ...
    const verificaEditavel = useCallback(() => {
        return prestacaoDeContas.status === 'EM_ANALISE';
    }, [prestacaoDeContas]);
 
    useEffect(() => {
        if(props?.state?.analisesDeContaDaPrestacao){
    ...
    const setAnaliseAtualUuidComPCAnaliseAtualUuid = useCallback(async () => {
        ...
        setTotalDespesasPeriodosAnterioresAjustes(undefined)
    ...
    }, [pcEmAnalise, setAnaliseAtualUuidComPCAnaliseAtualUuid])
    ...
    const possuiAbaConferenciaAtual = useMemo(() => pcEmAnalise, [pcEmAnalise]);
    const possuiAbaHistorico = useMemo(() => totalAnalisesDePcDevolvidas > 0, [totalAnalisesDePcDevolvidas]);
 
    useEffect(() => {
        setAbaAtivaResumoDosAcertos((abaAtual) => {
            if (!possuiAbaConferenciaAtual && !possuiAbaHistorico) {
                return "";
            }
 
            if (possuiAbaConferenciaAtual && possuiAbaHistorico) {
                if (!abaSelecionadaManualmente) {
                    return "conferencia-atual";
                }
 
                return abaAtual === "historico" || abaAtual === "conferencia-atual"
                    ? abaAtual
                    : "conferencia-atual";
            }
 
            if (possuiAbaConferenciaAtual) {
                return "conferencia-atual";
            }
    ...
    }, [possuiAbaConferenciaAtual, possuiAbaHistorico, abaSelecionadaManualmente]);
    ...
    const setPrimeiraAnalisePcDevolvida = useCallback(() => {
        ...
        setTotalDespesasPeriodosAnterioresAjustes('')
    ...
    }, [pcEmAnalise, setPrimeiraAnalisePcDevolvida])
    ...
    const verificaQtdeLancamentosDocumentosAjustes = useCallback(async () => {
        setLoading(true)
        if (infoAta?.contas && infoAta.contas.length > 0 && analiseAtualUuid) {
            const totais = await Promise.all(
                infoAta.contas.map(async (conta) => {
                    const contaUuid = conta.conta_associacao.uuid;
                    const [
                        extratosAjustes,
                        lancamentosAjustes,
                        documentosAjustes,
                        despesasPeriodosAnterioresAjustes
                    ] = await Promise.all([
                        getExtratosBancariosAjustes(analiseAtualUuid, contaUuid),
                        getLancamentosAjustes(analiseAtualUuid, contaUuid),
                        getDocumentosAjustes(analiseAtualUuid, contaUuid),
                        getDespesasPeriodosAnterioresAjustes(analiseAtualUuid, contaUuid)
                    ]);
 
                    return {
                        totalExtratos: extratosAjustes ? 1 : 0,
                        totalLancamentos: lancamentosAjustes.length,
                        totalDocumentos: documentosAjustes.length,
                        totalDespesasPeriodosAnteriores: despesasPeriodosAnterioresAjustes.length
                    };
                })
            );
 
            const totalExtratos = totais.reduce((acumulado, atual) => acumulado + atual.totalExtratos, 0);
            const totalLancamentos = totais.reduce((acumulado, atual) => acumulado + atual.totalLancamentos, 0);
            const totalDocumentos = totais.reduce((acumulado, atual) => acumulado + atual.totalDocumentos, 0);
            const totalDespesasPeriodosAnteriores = totais.reduce(
                (acumulado, atual) => acumulado + atual.totalDespesasPeriodosAnteriores,
                0
            );
 
            setTotalExtratosAjustes(totalExtratos)
            setTotalLancamentosAjustes(totalLancamentos)
            setTotalDocumentosAjustes(totalDocumentos)
            setTotalDespesasPeriodosAnterioresAjustes(totalDespesasPeriodosAnteriores)
        } else {
            setTotalExtratosAjustes(0)
            setTotalLancamentosAjustes(0)
            setTotalDocumentosAjustes(0)
            setTotalDespesasPeriodosAnterioresAjustes(0)
        }
    ...
    const msgNaoExistemSolicitacoesDeAcerto = useMemo(() => {
        if ((totalLancamentosAjustes > 0 || totalDocumentosAjustes > 0 || totalExtratosAjustes > 0 || totalDespesasPeriodosAnterioresAjustes > 0)){   
            return null;
        } else if (prestacaoDeContas?.devolucoes_da_prestacao && prestacaoDeContas.devolucoes_da_prestacao.length > 0) {
        ...
        <div className="page-content-inner">
            ...
            {loading || !analiseAtualUuid ? (
                ...
                <TabsConferenciaAtualHistorico
                        ...
                        abaAtiva={abaAtivaResumoDosAcertos}
                        setAbaAtiva={setAbaAtivaResumoDosAcertos}
                        setAbaSelecionadaManualmente={setAbaSelecionadaManualmente}
                    />                            
            )}
        </div>

...
const ModalVisualizarArquivoDeReferencia = ({
    show,
    handleClose,
    uuidArquivoReferencia,
    nomeArquivoReferencia,
    tipoArquivoReferencia,
}) => {
    const exibeArquivoDeReferencia = async () => {
        try {
            await getVisualizarArquivoDeReferencia(
                nomeArquivoReferencia,
                uuidArquivoReferencia,
                tipoArquivoReferencia,
            );
        } catch (e) {
            toast.error(`O carregamento do arquivo falhou.`);
    ...
    const bodyTextarea = () => {
        ...
        return (
            <>
                ...
                >
                    Este navegador não suporta a visualização de PDFs diretamente. Por favor, faça o
                    download do arquivo.
                </object>
            </>
        );

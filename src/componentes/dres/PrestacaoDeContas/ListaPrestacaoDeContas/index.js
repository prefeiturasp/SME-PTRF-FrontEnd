...
export const ListaPrestacaoDeContas = () => {
  ...
  const carregaPrestacoesDeContas = async () => {
    setLoading(true);
    if (periodoEscolhido) {
      let data_inicio = stateFiltros.filtrar_por_data_inicio
        ? moment(new Date(stateFiltros.filtrar_por_data_inicio), "YYYY-MM-DD").format("YYYY-MM-DD")
        : "";
      let data_fim = stateFiltros.filtrar_por_data_fim
        ? moment(new Date(stateFiltros.filtrar_por_data_fim), "YYYY-MM-DD").format("YYYY-MM-DD")
        : "";
      ...
      if (selectedStatusPc.includes("TODOS") || selectedStatusPc.length <= 0) {
        prestacoes_de_contas = await getPrestacoesDeContasTodosOsStatus(
          periodoEscolhido,
          stateFiltros.filtrar_por_termo,
          stateFiltros.filtrar_por_tipo_de_unidade,
        );
      } else if (
        (selectedStatusPc.length === 1 && selectedStatusPc.includes("NAO_APRESENTADA")) ||
        (selectedStatusPc.length === 2 &&
          selectedStatusPc.includes("NAO_APRESENTADA") &&
          selectedStatusPc.includes("NAO_RECEBIDA"))
      ) {
        prestacoes_de_contas = await getPrestacoesDeContasNaoRecebidaNaoGerada(
          periodoEscolhido,
          stateFiltros.filtrar_por_termo,
          stateFiltros.filtrar_por_tipo_de_unidade,
          array_status_convertido,
        );
      } else if (!selectedStatusPc.includes("NAO_APRESENTADA")) {
        prestacoes_de_contas = await getPrestacoesDeContas(
          periodoEscolhido,
          stateFiltros.filtrar_por_termo,
          stateFiltros.filtrar_por_tipo_de_unidade,
          array_status_convertido,
          stateFiltros.filtrar_por_tecnico_atribuido,
          data_inicio,
          data_fim,
        );
      } else {
  ...
  const statusTemplate = (rowData) => {
    return (
      <div>
        {rowData["status"] ? (
          <span className={`span-status-${rowData["status"]}`}>
            <strong>{exibeLabelStatus(rowData["status"]).texto_col_tabela}</strong>
          </span>
        ) : (
          ""
        )}
  ...
  const dataTemplate = (rowData, column) => {
    return <div>{rowData[column.field] ? moment(rowData[column.field]).format("DD/MM/YYYY") : "-"}</div>;
  };

  const nomeTemplate = (rowData) => {
    return (
      <div>
        {rowData["unidade_nome"] ? (
          <strong>
            {rowData["unidade_tipo_unidade"]} {rowData["unidade_nome"]}
          </strong>
        ) : (
          ""
        )}
  ...
  const seiTemplate = (rowData) => {
    return <div>{rowData["processo_sei"] ? <span>{rowData["processo_sei"]}</span> : "-"}</div>;
  };

  const tecnicoTemplate = (rowData, column) => {
    return <div>{rowData[column.field] ? rowData[column.field] : "-"}</div>;
  };
    ...
    return (
      <div>
        {rowData.status !== "NAO_APRESENTADA" ? (
          ...
          >
            <FontAwesomeIcon style={{ marginRight: "0" }} icon={getIcone(rowData.status)} />
          </Link>
        ) : (
          <button onClick={() => gravaPcNaoApresentada(rowData)} className="btn btn-link">
            <FontAwesomeIcon style={{ marginRight: "0" }} icon={faEye} />
          </button>
        )}
      </div>
    );
    ...
    <PaginasContainer>
      ...
      {loading ? (
        ...
        <div className="page-content-inner">
          ...
          />
          <BarraDeStatus qtdeUnidadesDre={qtdeUnidadesDre} prestacaoDeContas={prestacaoDeContas} />
          ...
          {prestacaoDeContas && prestacaoDeContas.length > 0 ? (
            ...
            <MsgImgLadoDireito texto="Nenhuma prestação retornada. Tente novamente com outros filtros" img={Img404} />
          )}
        </div>
      )}
    </PaginasContainer>

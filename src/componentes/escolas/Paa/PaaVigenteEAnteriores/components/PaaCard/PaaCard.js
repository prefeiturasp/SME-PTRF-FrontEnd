...
import RetificacoesAnteriores from "./RetificacoesAnteriores";
...
export const PaaCard = ({ dados, onDadosAtualizados }) => {
        ...
        async (bloco) => {
            try {
                await downloadDocumentoFinalPaa(paaUuid, {
                    retificacao: bloco.status.retificacao,
                });
            } catch (error) {
                if (error.status === 404) {
                    toastCustom.ToastCustomError(
                        "Documento não encontrado",
                        "Não foi possível encontrar documento para visualização.",
                    );
                }
            }
        },
    ...
    const statusGeracaoAtaRetificacao =
        dados?.retificacao?.ata?.status?.status_geracao;
    useEffect(() => {
        ...
        statusGeracaoAtaApresentacao,
        statusGeracaoAtaRetificacao,
        onDadosAtualizados,
    ]);

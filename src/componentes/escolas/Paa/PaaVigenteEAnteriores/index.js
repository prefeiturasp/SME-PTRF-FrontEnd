...
import CancelarRetificacao from "../componentes/CancelarRetificacao";
...
export const PaaVigenteEAnteriores = () => {
    ...
    const exibirBotaoRetificar = useMemo(() =>
        podeExibirBotaoRetificar(vigente),
    );

    return (
        <PaginasContainer>
            <BreadcrumbComponent items={BREADCRUMB_PAA} />
            <h1 className="titulo-itens-painel mt-5">
                Plano Anual de Atividades
            </h1>

            {!temAssociacao && (
                <div className="mt-4 paa-vigente-e-anteriores__alerta">
                    Associação não identificada. Selecione uma unidade para
                    carregar o Plano Anual de Atividades.
                </div>
            )}
            ...
            {exibirConteudoPrincipal && (
                <div className="page-content-inner rounded">
                    ...
                    {!vigente && (
                        <PaaListaVazia>
                            <>
                                A sua unidade ainda não possui Plano Vigente.{" "}
                                <br /> Clique no menu &quot;Elaboração&quot;
                                para iniciar o Plano Anual Vigente
                            </>
                        </PaaListaVazia>
                    )}
                    ...
                    <div className="mt-5">
                        <h3 className="paa-vigente-e-anteriores__titulo-secao">
                            Planos anteriores
                        </h3>

                        {anteriores.length === 0 && (
                            <PaaListaVazia>
                                A sua unidade ainda não possui Planos
                                anteriores.
                            </PaaListaVazia>
                        )}
                        ...
                        {anteriores.map((paaAnterior) => {
                            ...
                            return (
                                <div className="mt-3" key={paaAnterior.uuid}>
                                    <PaaCardBarraTitulo
                                        titulo={tituloCardPaa(
                                            paaAnterior?.referencia,
                                        )}
                                        isAberto={!!isOpen}
                                        onToggle={() =>
                                            toggleAnterior(paaAnterior.uuid)
                                        }
                                    />
                                    {isOpen && (
                                        <PaaCard

...
describe("PaaCard", () => {
    ...
    it("renderiza seção única quando não está em retificação", () => {
        ...
        expect(
            screen.getAllByText("Plano Anual").length,
    ...
    it("agenda polling de onDadosAtualizados quando ata está EM_PROCESSAMENTO", () => {
        ...
        render(
            <PaaCard dados={dados} onDadosAtualizados={onDadosAtualizados} />,
        );
        act(() => {
    ...
    it("abre modal de PDF ao visualizar plano", async () => {
        ...
        render(<PaaCard dados={dados} />);
        const planoHeading = screen.getByRole("heading", {
            name: "Plano Anual",
        });
        const [btnVisualizarPlano] = within(
            planoHeading.parentElement,
        ).getAllByRole("button");
        await act(async () => {
    ...
    it("chama download do documento final ao clicar em download do plano", async () => {
        ...
        const botoesPlano = within(planoHeading.parentElement).getAllByRole(
            "button",
        );
        await act(async () => {
            fireEvent.click(botoesPlano[1]);
        });
        expect(downloadDocumentoFinalPaa).toHaveBeenCalledWith("paa-dl", {
            retificacao: false,
        });
    });
    ...
    it("chama getDownloadAtaPaa ao baixar ata", async () => {
        ...
        render(<PaaCard dados={dados} />);
        const ataHeading = screen.getByRole("heading", {
            name: "Ata de apresentação do PAA",
        });
        const botoesAta = within(ataHeading.parentElement).getAllByRole(
            "button",
        );
        await act(async () => {
    ...
    it("renderiza bloco de Retificações anteriores", () => {
        const dados = {
            uuid: "paa-uuid",
            esta_em_retificacao: true,
            exibe_dados_retificacao: true,
    ...
    it("renderiza bloco de Retificações anteriores e Retificação Anterior", () => {
        const dados = {
            uuid: "paa-uuid",
            esta_em_retificacao: true,
            exibe_dados_retificacao: true,
            retificacao: {
                secao_titulo: "PAA Retificado #3",
                documento: {
                    ...DOCUMENTO_PADRAO,
                    status: {
                        status_geracao: "CONCLUIDO",

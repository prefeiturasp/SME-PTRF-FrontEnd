...
const ReceitasPrevistasPTRF = ({ paa }) => {
  ...
  const { data: totalRecursosProprios } = useGetTotalizadorRecursoProprio(paa.associacao, paa.uuid);
  ...
  const carregaPaa = useCallback(async ()=>{
    queryClient.invalidateQueries({ queryKey: [`retrieve-paa`, paa.uuid], exact: false })

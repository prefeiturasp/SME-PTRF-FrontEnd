...
jest.mock(
    "../../../../../../services/escolas/PrestacaoDeContas.service",
    () => ({
...
jest.mock("../NovoFormularioEditaAta", () => ({
    NovoFormularioEditaAta: () => <div data-testid="novo-formulario" />,
}));
...
const renderEdicaoAtaPaa = () => {
    return render(
        <MemoryRouter initialEntries={["/paa/123"]}>
            <Routes>
                <Route path="/paa/:uuid_paa" element={<EdicaoAtaPaa />} />
            </Routes>
        </MemoryRouter>,
    );
};
...
describe("EdicaoAtaPaa - carregamento da lista de presentes", () => {
    ...
    it("não deve adicionar novamente um membro que já existe na lista padrão", async () => {
        ...
        getListaPresentesPaa.mockResolvedValue([membro]);

        getListaPresentesPadraoPaa.mockResolvedValue([membro]);
        ...
        await waitFor(() => {
            ...
            expect(
                lista.filter(
                    (participante) => participante.identificacao === "111111",
                ),
            ).toHaveLength(1);
        });
    });
    ...
    it("deve manter o professor do grêmio quando ele for um não membro", async () => {
        ...
        getListaPresentesPaa.mockResolvedValue([professorGremio]);
    ...
    it("deve adicionar todos os membros novos e os não membros sem duplicar membros existentes", async () => {
        ...
        await waitFor(() => {
            ...
            expect(
                lista.filter(
                    (participante) => participante.identificacao === "222222",
                ),
            ).toHaveLength(1);

            expect(
                lista.filter(
                    (participante) => participante.identificacao === "333333",
                ),
            ).toHaveLength(1);

            expect(
                lista.filter(
                    (participante) => participante.identificacao === "444444",
                ),
            ).toHaveLength(1);
    ...
    it("deve permitir adicionar um participante diferente quando ambos não possuem identificação", async () => {
        const listaPresentesAta = [
            {
                identificacao: "",
                nome: "Maria",

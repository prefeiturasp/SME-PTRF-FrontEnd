...
describe("FormularioEditaAta - professor orientador do grêmio", () => {
    ...
    it("deve permitir adicionar um participante diferente quando ambos não possuem identificação", async () => {
        ...
        const { container } = render(
            <FormularioEditaAta
                {...propsBase}
                listaPresentesPadrao={[participanteExistente]}
                precisaProfessorGremio={false}
                formRef={{
                    current: {
                        values: {
        ...
        fireEvent.click(screen.getByText("+ Adicionar presente"));
        const campoNome = await waitFor(() =>
            container.querySelector(
                'input[name="listaPresentesPadrao[1].nome"]',
            ),
        );

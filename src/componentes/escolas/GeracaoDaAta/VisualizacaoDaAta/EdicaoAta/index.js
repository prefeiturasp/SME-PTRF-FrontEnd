...
export const EdicaoAta = () => {
  ...
  const periodo_prestacao_de_contas = JSON.parse(
    localStorage.getItem("periodoPrestacaoDeConta")
  );
  const periodoUuid = periodo_prestacao_de_contas
    ? periodo_prestacao_de_contas.periodo_uuid
    : "";

  // Hooks Personalizados
  const repassesPendentes = useCarregaRepassesPendentesPorPeriodoAteAgora(
    uuid_associacao,
    periodoUuid
  );
  ...
  const [showModalAvisoRegeracaoAta, setShowModalAvisoRegeracaoAta] =
    useState(false);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      let participantesNaoMembros = listaPresentesAta.filter(
  ...
  const getDadosAta = async () => {
    ...
    setStateFormEditarAta({
      ...
      justificativa_repasses_pendentes:
        dados_ata.justificativa_repasses_pendentes,
    });
  ...
  const editaStatusDePresencaMembro = (identificacao) => {
    ...
    const membroListaPresentesSelecionado = copiaListaPresentesPadrao.find(
      (membro) => membro.identificacao === identificacao
    );

    if (membroListaPresentesSelecionado) {
      membroListaPresentesSelecionado.presente =
        !membroListaPresentesSelecionado.presente;
    }
  ...
  const onSubmitFormEdicaoAta = async () => {
    ...
    let data_da_reuniao = dadosForm.stateFormEditarAta.data_reuniao
      ? moment(dadosForm.stateFormEditarAta.data_reuniao).format("YYYY-MM-DD")
      : null;
    let hora_reuniao = dadosForm.stateFormEditarAta.hora_reuniao
      ? moment(dadosForm.stateFormEditarAta.hora_reuniao, "HHmm").format(
          "HH:mm"
        )
      : "00:00";
    ...
    if (visoesService.featureFlagAtiva("historico-de-membros")) {
      ...
      let listaParticipantes = adicionaAtaUuidAosParticipantes(
        dadosForm.listaParticipantes
      );
      ...
      payload = {
        ...
        justificativa_repasses_pendentes:
          dadosForm.stateFormEditarAta.justificativa_repasses_pendentes,
      };
    } else {
      payload = {
        cargo_presidente_reuniao:
          dadosForm.stateFormEditarAta.cargo_presidente_reuniao,
        cargo_secretaria_reuniao:
          dadosForm.stateFormEditarAta.cargo_secretaria_reuniao,
        comentarios: dadosForm.stateFormEditarAta.comentarios,
        convocacao: dadosForm.stateFormEditarAta.convocacao,
        data_reuniao: data_da_reuniao,
        local_reuniao: dadosForm.stateFormEditarAta.local_reuniao,
    ...
    try {
      ...
      let tipo_ata =
        dadosAta.tipo_ata === "RETIFICACAO" ? "retificação" : "apresentação";
      if (response.pdf_gerado_previamente) {
        setShowModalAvisoRegeracaoAta(true);
      }
      toastCustom.ToastCustomSuccess(
        "Ata salva com sucesso",
        `As edições da ata de ${tipo_ata} foram salvas com sucesso.`
      );
    } catch (e) {

...
describe("MandatoInfo", () => {
    ...
    it("deve renderizar o mandato recebido por prop, sem depender do mandato vigente", () => {
        useGetMandatoVigente.mockReturnValue({
            isLoading: true,
            data: null,
        });
        ...
        render(
            <MandatoInfo
                mandato={{ uuid: "456", data_inicial: "2023-01-01", data_final: "2023-12-31" }}
            />
        );

        expect(screen.getByText(/mandato atual:/i)).toBeInTheDocument();
        expect(
            screen.getByText("01/01/2023 até 31/12/2023", { exact: false })
        ).toBeInTheDocument();
    });

    it("deve usar o label customizado quando informado", () => {
        useGetMandatoVigente.mockReturnValue({ isLoading: false, data: null });

        render(
            <MandatoInfo
                mandato={{ uuid: "456", data_inicial: "2023-01-01", data_final: "2023-12-31" }}
                label="Mandato selecionado"
            />
        );

        expect(screen.getByText(/mandato selecionado:/i)).toBeInTheDocument();
        expect(screen.queryByText(/mandato atual:/i)).not.toBeInTheDocument();
    });
});

...
import { formatDateOrDash } from "../../../../../utils/FormataData";
...
const TabelaTransacoes = ({
  ...
  const dataTemplate = (rowData = null, column = null, data = null) => {
    ...
    if (rowData && rowData.notificar_dias_nao_conferido && rowData.notificar_dias_nao_conferido > 0) {
      return (
        <div>
          <span style={{ color: "black" }}>{formatDateOrDash(data_para_verificar)}</span>
        </div>
      );
    } else {
      return <span>{formatDateOrDash(data_para_verificar)}</span>;
    }
  };
  ...
  const valorTemplate = (rowData = null, column = null, valor = null) => {
    ...
    return (
      <div>
        <span>{valor_formatado}</span>
      </div>
    );
  };

  const rowExpansionTemplate = (data) => {
    return despesaTemplate(data);
  };
    ...
    return (
      <div className="align-middle text-center">
        <input
          ...
          onChange={(e) =>
            handleChangeCheckboxTransacoes(e, rowData.documento_mestre.uuid, true, rowData.tipo_transacao)
          }
          name="checkConferido"
          id="checkConferido"
          data-testid="checkConferido"
    ...
    return (
      <div style={{ marginTop: "1.3rem" }}>
        <input
          ...
          name="checkConferidoRateio"
          id="checkConferidoRateio"
          data-testid="checkConferidoRateio"
          disabled={periodoFechado || !PERMISSAO_CHANGE_CONCILIACAO_BANCARIA}
        />
      </div>
    );
  ...
  const tagDataTemplate = (rateio) => {
    return (
      <>
        {rateio.tag ? (
          <span className="badge badge-primary p-1" style={{ backgroundColor: "#086397" }}>
            {rateio.tag.nome}
          </span>
        ) : (
          <span>-</span>
        ...
        <div className="border pb-2">
          <div className="row pl-3 pr-3">
            ...
            <div className="col border-left border-bottom p-2">
              ...
              {data.documento_mestre.tipo_documento && data.documento_mestre.tipo_documento.nome
            ...
            <div className="col border-left border-bottom p-2">
              ...
              {data.documento_mestre.tipo_transacao && data.documento_mestre.tipo_transacao.nome
            ...
            {data.documento_mestre.tipo_transacao &&
            data.documento_mestre.tipo_transacao.nome &&
            ...
            data.rateios.map((rateio, index) => (
              <div key={index} className="border-bottom pb-2">
                ...
                <div className="row pl-4 pr-4" key={`${index}-info-rateio-top`}>
                  ...
                  <div className="col-lg-3 border pb-2">
                    ...
                    {rateio.aplicacao_recurso
                      ? tabelasDespesa.tipos_aplicacao_recurso.find(
                ...
                <div className="row pl-4 pr-4" key={`${index}-info-rateio-bottom`}>
                  ...
                  <div className="col-lg-2 border align-middle text-center pb-2">{conferidoRateioTemplate(rateio)}</div>
                </div>
              </div>
            ))}
        </div>
  ...
  return loading ? (
    ...
    <>
      <div className="text-right">
        ...
        />
        <LimparArgumentosOrdenacao limparOrdenacao={limparOrdenacao} camposOrdenacao={camposOrdenacao} />
      </div>
      <div className="row mt-4">
        <div className="col-12">
          <div className="datatable-responsive-demo">
            ...
            >
              ...
              <Column field="data" header="Data" style={{ borderLeft: "none" }} body={dataTemplate} />
              <Column field="tipo_transacao" header="Tipo de lançamento" body={tipoLancamentoTemplate} />
              <Column
              ...
              <Column
                ...
                style={{ width: "15%" }}
              />
              <Column field="valor_transacao_na_conta" header="Valor (R$)" body={valorTemplate} />
              <Column
                field="conferido"
                header="Demonstrado"
        ...
        <section>
          <RedirectModalTabelaLancamentos show={showModal} handleClose={onHandleClose} onConfirm={onConfirm} />
        </section>
      </div>
    </>
  ) : (
    <Fragment>{emptyListComponent}</Fragment>
  );
};

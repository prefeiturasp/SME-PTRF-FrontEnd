...
export const TopoComBotaoVoltar = ({
  statusPc,
  prestacaoContaUuid,
  onClickVoltar,
  periodoFormatado,
  ...
  const handleVerificaAcertos = async () => {
    if (await temAcertosPendententes()) {
      ...
      localStorage.setItem(
        ...
        JSON.stringify({
          data_final: periodo.data_fim_realizacao_despesas,
          data_inicial: periodo.data_inicio_realizacao_despesas,
  ...
  return (
    <div className="d-flex bd-highlight align-items-center">
      <div className="flex-grow-1 bd-highlight texto-periodo-topo">
        <p className="mb-2">
          {periodoFormatado && periodoFormatado.referencia ? periodoFormatado.referencia : ""} -{" "}
          {periodoFormatado && periodoFormatado.data_inicio_realizacao_despesas
            ? periodoFormatado.data_inicio_realizacao_despesas
            : "-"}{" "}
          até{" "}
          {periodoFormatado && periodoFormatado.data_fim_realizacao_despesas
            ? periodoFormatado.data_fim_realizacao_despesas
            : "-"}
        </p>
        <h1 className="titulo-itens-painel">Devolução para acertos</h1>
      </div>
      <div className="p-2 bd-highlight">
        <button onClick={onClickVoltar} className="btn btn-outline-success ml-2">
      ...
      <div className="p-3 bd-highlight">
        {verificaSePodeIrParaConcluidas() ? (
          <button className="btn btn-success mr-2" onClick={handleVerificaAcertos}>

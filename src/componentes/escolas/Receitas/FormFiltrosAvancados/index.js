...
export const FormFiltrosAvancados = (props) => {
  ...
  const handleSubmit = async (event) => {
    ...
    let data_inicio = state.data_inicio
      ? moment(state.data_inicio, "YYYY-MM-DD").format("YYYY-MM-DD")
      : null;
    let data_fim = state.data_fim
      ? moment(state.data_fim, "YYYY-MM-DD").format("YYYY-MM-DD")
      : null;
    buscaTotaisReceitas(
      state.tipo_receita,
    ...
    const lista_retorno_api = await filtrosAvancadosReceitas(
    ...
    setLista(lista_retorno_api.results || []);
    setTotalRegistros(lista_retorno_api.count || 0);
    setPaginacaoAtual(1);
    ...
    mantemEstadoFiltrosUnidade.setEstadoFiltrosUnidadesUsuario(
      ...
      {
        filtros_receitas: {
          filtrar_por_termo: state.filtrar_por_termo,
          tipo_receita: state.tipo_receita,
          acao_associacao: state.acao_associacao,
          conta_associacao: state.conta_associacao,
          data_inicio,
          data_fim,
        },
      },
  ...
  const limpaFormulario = () => {
    ...
    mantemEstadoFiltrosUnidade.limpaEstadoFiltrosUnidadesUsuarioLogado(
      visoesService.getUsuarioLogin(),
    );
  };
  ...
  return (
    <div
      className={`row ${btnMaisFiltros ? "lista-de-receitas-visible" : "lista-de-receitas-invisible"}`}
    >
          ...
          <div className="form-row">
            <div className="form-group col-lg-6">
              ...
              >
                ...
                {tabelas.tipos_receita !== undefined &&
                tabelas.tipos_receita.length > 0
            ...
            <div className="col-12 col-lg-6">
              <div className="row">
                <div className="form-group col-12 col-md-6 pr-md-1">
                  <label htmlFor="filtrar_por_termo">
                    Detalhamento do crédito
                  </label>
                  <input
                    value={state.filtrar_por_termo}
                    onChange={(e) =>
                ...
                <div className="form-group col-12 col-md-6 pl-md-1">
                  ...
                  >
                    ...
                    {tabelas.contas_associacao !== undefined &&
                    tabelas.contas_associacao.length > 0
                      ? tabelas.contas_associacao.map((item, key) => (
                          <option key={key} value={item.uuid}>
                            {item.nome}{" "}
                            {item.solicitacao_encerramento
                              ? `- Conta encerrada em ${moment(item.solicitacao_encerramento.data_de_encerramento_na_agencia).format("DD/MM/YYYY")}`
                              : ""}
                          </option>
                        ))
                      : null}
            ...
            <div className="form-group col-12 col-lg-6">
              ...
              >
                ...
                {tabelas.acoes_associacao !== undefined &&
                tabelas.acoes_associacao.length > 0
                  ? tabelas.acoes_associacao.map((item, key) => (
                      <option key={key} value={item.uuid}>
                        {item.nome}
                      </option>
                    ))

...
export const YupSignupSchemaCadastroDespesa = yup.object().shape({
  cpf_cnpj_fornecedor: yup
    .string()
    .test('test-name', 'Digite um CPF ou um CNPJ válido', function (value) {
      if (value !== undefined) {
...
export const validateFormDespesas = (
  values,
  props /* only available when using withFormik */
) => {
  values.qtde_erros_form_despesa = document.getElementsByClassName(
    'is_invalid'
  ).length
  ...
  let var_valor_recursos_acoes =
    trataNumericos(values.valor_total) -
    trataNumericos(values.valor_recursos_proprios)
  ...
  values.rateios.map((rateio) => {
    if (rateio.aplicacao_recurso === 'CAPITAL') {
      var_valor_total_dos_rateios_capital =
        var_valor_total_dos_rateios_capital +
        trataNumericos(rateio.quantidade_itens_capital) *
          trataNumericos(rateio.valor_item_capital)
    } else {
      var_valor_total_dos_rateios_custeio =
        var_valor_total_dos_rateios_custeio +
        trataNumericos(rateio.valor_rateio)
    }
  })

  var_valor_total_dos_rateios =
    var_valor_total_dos_rateios_capital + var_valor_total_dos_rateios_custeio

  if (
    round(var_valor_recursos_acoes, 2) !== round(var_valor_total_dos_rateios, 2)
  ) {
    errors.valor_recusos_acoes =
      'O total das classificações deve corresponder ao valor total da nota'
  }
  return errors
}
...
export const validaPayloadDespesas = (values) => {
  ...
  if (
    typeof values.tipo_documento === 'object' &&
  ...
  if (
    typeof values.tipo_transacao === 'object' &&
  ...
  values.valor_recursos_proprios = trataNumericos(
    values.valor_recursos_proprios
  )
  ...
  values.rateios.map((rateio) => {
    if (
      typeof rateio.especificacao_material_servico === 'object' &&
      rateio.especificacao_material_servico !== null
    ) {
      rateio.especificacao_material_servico =
        rateio.especificacao_material_servico.id
    } else {
      rateio.especificacao_material_servico = convertToNumber(
        rateio.especificacao_material_servico
      )
    }

    if (
      typeof rateio.conta_associacao === 'object' &&
      rateio.conta_associacao !== null
    ) {
      rateio.conta_associacao = rateio.conta_associacao.uuid
    } else {
      if (
        rateio.conta_associacao === '0' ||
        rateio.conta_associacao === '' ||
        rateio.conta_associacao === 0
      ) {
        rateio.conta_associacao = null
      }
    ...
    rateio.quantidade_itens_capital = convertToNumber(
      rateio.quantidade_itens_capital
    )
    ...
    if (
      rateio.especificacao_material_servico === '0' ||
      rateio.especificacao_material_servico === 0 ||
      rateio.especificacao_material_servico === ''
    ) {
      rateio.especificacao_material_servico = null
    ...
    if (rateio.aplicacao_recurso === 'CAPITAL') {
      rateio.valor_rateio = round(
        rateio.quantidade_itens_capital * rateio.valor_item_capital,
        2
      )
    }
  })
...
export const exibeDataPT_BR = (data) => {
  if (data === 'None') {
    ...
    data = moment(new Date(data), 'YYYY-MM-DD')
      .add(1, 'days')
      .format('DD/MM/YYYY')
  }
  return data
...
export const calculaValorRecursoAcoes = (props) => {
  ...
  let valor_recursos_propriosTratado = trataNumericos(
    props.values.valor_recursos_proprios
  )
